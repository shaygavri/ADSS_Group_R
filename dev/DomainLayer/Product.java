...
public class Product {
    ...
    public void displayProduct(Product p, boolean showHeader) {
        if (showHeader) {
            System.out.println("\n" + String.format("%-6s | %-15s | %-8s | %-4s | %-4s | %-4s | %-4s | %-6s | %-6s | %-10s | %-10s | %-5s", "ID", "Product Name", "Cat", "Shop", "Whs", "Dmg", "Total", "Cost", "Price", "Expiry", "Added", "Loc"));
            System.out.println("-".repeat(120));
        }

        System.out.printf("%-6s | %-15.15s | %-8.8s | %-4d | %-4d | %-4d | %-4d | %-6.1f | %-6.1f | %-10s | %-10s | %-5s\n", p.getProductID(), p.getProductName(), p.getCategory(), p.getShopQuantity(), p.getWarehouseQuantity(), p.getDamagedAmount(), p.getTotalQuantity(), p.getCost_price(), p.getBest_price(), p.getExpirationDate(), p.getEntryTime(), p.getAisle() + "-" + p.getShelf());
    }
}